...
struct __EkcfiAttrS2 {
    ...
    text_mutex_addr: u64,
    within_blacklist_addr: u64,
}

#[repr(C)]
#[derive(Debug, Clone, Copy)]
struct __EkcfiAttrS3 {
    prog_fd: u64,
    index: u32,
}
...
#[inline(always)]
fn is_cs_jmp(vals: &[u8]) -> usize {
    if vals.len() <= CSJMP.len() {
        return 0;
    }
    
    if vals[0] == CSJMP[0] && vals[1] == CSJMP[1] {
        let mut rel: usize = 0;
        for it in vals[2..].iter().rev() {
            rel <<= 8;
            rel += *it as usize;
        }
        return rel;
...
struct Args {
    ...
    /// 0: Indirect call, 1: Return, 2: Both
    choice: u8,
...
fn main() {
    ...
    for (offset, _) in section_data.iter().enumerate() {
        ...
        if offset + 5 < section_data_len && is_nop5(&section_data[offset..offset + 5]) {
            if offset + 11 < section_data_len {
               let rel_offset = is_cs_jmp(&section_data[offset + 5..offset + 11]);
               let index = (offset + 5 + rel_offset + 6) % section_data_len;
               if rel_offset > 0 && is_ret(section_data[index]) {
                    /* Looks dirty */
                    ret_addrs.push(offset as u64 + text_shdr.sh_addr);
               } else {
                    ind_addrs.push(offset as u64 + text_shdr.sh_addr);
               }
            } else {
               ind_addrs.push(offset as u64 + text_shdr.sh_addr);
            }
    ...
    if choice == 0 || choice == 2 {
        let mut attr1: ekcfi_attr = unsafe { mem::zeroed() };
        attr1.__s1.addrs = ind_addrs.as_ptr();
        attr1.__s1.len = ind_addrs.len() as u64;
        attr1.__s1.tbli = 0;

        println!("Found {} indirect thunks", ind_addrs.len());
    
        // Load table 1
        unsafe {
            ekcfi_ctl(EKCFI_CMD::EKCFI_LOAD_TBL, &mut attr1)
                .or_else(|e| syscall_perror(e, "ekcfi_ctl\0".as_ptr() as *const i8))
                .unwrap();
        }
    }
    ...
    if choice == 1 || choice == 2 {
        let mut attr2: ekcfi_attr = unsafe { mem::zeroed() };
        attr2.__s1.addrs = ret_addrs.as_ptr();
        attr2.__s1.len = ret_addrs.len() as u64;
        attr2.__s1.tbli = 1;
    
        println!("Found {} return thunks", ret_addrs.len());

        // Load table 2
        unsafe {
            ekcfi_ctl(EKCFI_CMD::EKCFI_LOAD_TBL, &mut attr2)
                .or_else(|e| syscall_perror(e, "ekcfi_ctl\0".as_ptr() as *const i8))
                .unwrap();
        }
    }
    ...
    unsafe {
        ...
        attr.__s2.within_blacklist_addr = kallsyms["within_kprobe_blacklist"].get_base_addr();
