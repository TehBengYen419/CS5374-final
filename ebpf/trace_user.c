...
int main(int argc, char *argv[])
{
	...
	// For training CFG
	map_fd = bpf_object__find_map_fd_by_name(obj, "sample_map");
	if (map_fd < 0) {
		printf("ERROR: finding call_map in object file failed\n");
		ret = 1;
		goto obj_close;
	}
	for (i = 0; i < ARRAY_LEN(sample_trace); i++) {
		unsigned callee_key = (unsigned)(sample_trace[i].callee & 0xFFFFFFFF);
		unsigned long long *callers = sample_trace[i].callers;
	...
	if (index == 0)
		prog = bpf_object__find_program_by_name(obj, "ebpf_trace_ind_ekcfi_check");
	else if (index == 1)
		prog = bpf_object__find_program_by_name(obj, "ebpf_trace_ret_ekcfi_check");
	if (!prog) {
	...
	if (ekcfi_attach(prog_fd, index) < 0) {
